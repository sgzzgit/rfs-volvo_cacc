...
//static int test_site = RFS;  //   CRO, NVD// moved to coording.h on 04_14_11
static int rd_info_sw=(int)GRADE_POSTMILE;   // GRADE_GPS             
...
static local_gps_typ EB_start, WB_start, gps_trk;
static local_gps_typ *EB_start_pt;
static local_gps_typ *WB_start_pt;
...
//    unsigned short j=0;
 ...
 //   verbose_flag=TRUE;
    ...
    config.max_spd=((float)get_ini_double(pfin1, "MaxSpeed", 15.0 ))*mph2mps;
   ...
   // From veh_long.h
     ...
     con_state. split_f_dist=DES_FOLLOW_DIST+SPLIT_DIST;
     if (con_state. split_f_dist<4.0)
     	con_state. split_f_dist=4.0;
     con_state. join_f_dist=DES_FOLLOW_DIST-JOIN_DIST;
     ...
     memset(&road_info,0,sizeof( road_info));  
       ...
       road_info. direction = config.dir;
    ...
    if( test_site == RFS )
     {
	    ...
	    if (config_pt-> task == 0)                          // Determine  Max_spd from realtime.ini
	    {
		        
          		config.max_spd=(15.0)*mph2mps;   
        }
        if (config_pt-> task == 1)                          // Determine  Max_spd from realtime.ini
	    {
		      
          		config.max_spd=(15.0)*mph2mps;   
          		if (config_pt-> run == 1)
          	   {
          	  		 if (vehicle_info_pt -> run_dist > 70.0)               // Using lead veh run_dist for coordination
	          	  		 manager_cmd_pt-> f_manage_index = 2;
  	           }
  	           if (config_pt-> run == 2)
          	   {
          	    	if (vehicle_info_pt -> run_dist > 100.0)             // Using lead veh run_dist for coordination
          	    		manager_cmd_pt-> f_manage_index = 3;
  	        	}  
        }
            
          // task 3:  Splitting simultaneously; 4: Joining simultaneously; 5:  Combined Simultaneous Splitting and Joining
           // 6:  Splitting individually; 7: Joining individually; 8:  Combined Individual Splitting and Joining
         if ((config_pt-> task == 3) || (config_pt-> task == 4) || (config_pt-> task == 6) || (config_pt-> task == 7) )      
         {		        
		        config.max_spd=(15.0)*mph2mps;       
		        // Using default DES_FOLLOW_DIST	
         }
  ...
  if( test_site == NVD )
    { 
	     track_length = 5.0*1609.0;                        //For Nevada test from 08_31_10
	   ...
	   if (config_pt-> task == 0)                          // Determine  Max_spd from realtime.ini
	       {   
		        /*if (config_pt-> run == 1)
      		    ...
      		    if (config_pt-> run == 3)
		        	con_state. des_f_dist=7.0;  
		        if (config_pt-> run == 4)
		        	con_state. des_f_dist=6.0; 
		        if (config_pt-> run == 5)
		        	con_state. des_f_dist=5.0;  	 	
          	   config.max_spd=(config.max_spd)*mph2mps;  */
          	   config.max_spd=55*mph2mps; 
           } // task 0 end
          
         if (config_pt-> task == 1)            //  Emergency handling
           {	
	                 		     		          		 		         	
          	    config.max_spd=(53.0)*mph2mps;   
          	   if (config_pt-> run == 1)
          	   {
          	  		 if (vehicle_info_pt -> run_dist > 3000.0)               // Using lead veh run_dist for coordination
          	        		manager_cmd_pt-> f_manage_index = 2;
  	           }
  	           if (config_pt-> run == 2)
          	   {
          	    		if (vehicle_info_pt -> run_dist > 3500.0)             // Using lead veh run_dist for coordination
          	    			manager_cmd_pt-> f_manage_index = 3;
  	        	}  
  	        	if (config_pt-> run == 3)
          	   {
          	  		 if (vehicle_info_pt -> run_dist > 2500.0)                 // Using lead veh run_dist for coordination
          	        		manager_cmd_pt-> f_manage_index = 2;
      	        	if (vehicle_info_pt -> run_dist > 3500.0)                   // Using lead veh run_dist for coordination
          	    			manager_cmd_pt-> f_manage_index = 3;
  	           } 
           ...
           // task 3:  Splitting simultaneously; 4: Joining simultaneously; 5:  Combined Simultaneous Splitting and Joining
           // 6:  Splitting individually; 7: Joining individually; 8:  Combined Individual Splitting and Joining
         if (config_pt-> task == 3 || (config_pt-> task == 4)   || (config_pt-> task == 5)   || (config_pt-> task == 6)   || (config_pt-> task == 7)  || (config_pt-> task == 8)   )                                                 
         {
	            if (config_pt-> run == 1)
		            config.max_spd=30.0*mph2mps;  
      		    if (config_pt-> run == 2)
	      		    config.max_spd=40.0*mph2mps;  
      		    if (config_pt-> run == 3)
	      		    config.max_spd=48.0*mph2mps;  
         }      // task 3 end
         
        
         if (config_pt-> task == 9)                         // Emergency stop                        
         {
	            if (config_pt-> run == 1)
		            config.max_spd=30.0*mph2mps;  
      		    if (config_pt-> run == 2)
       ...
       if (config_pt-> task == 10)                                                  // Grading in two directions
         {
	            track_length = 14.15*1609.0;                                     // For grading
        ...
        if (config.max_spd > 55.0*mph2mps)               // Protection in [m/s]
        ...
        if (config_pt-> task == 2)
        {
	        if (test_site == RFS)
	         {
	            stop_period=2.0*(15.0*mph2mps) / (config.max_dcc);                                          // Used after Nov. 25 02
        		stop_dist = track_length - ((15.0*mph2mps)*stop_period - 0.25*(config.max_dcc)*stop_period*stop_period);    
    		}   
	        if (test_site == NVD)
	        {
	            stop_period=2.0*(53.0*mph2mps) / (config.max_dcc);                                          // Used after Nov. 25 02
        		stop_dist = track_length - ((53.0*mph2mps)*stop_period - 0.25*(config.max_dcc)*stop_period*stop_period);    
    		}   
        }
        else
        {
        ...
        if ((config_pt-> task == 1) || (config_pt-> task == 3 ) || (config_pt-> task == 6 ))  
        		manager_cmd_pt-> stop_dist=stop_dist;    
        else
        	manager_cmd_pt-> stop_dist=stop_dist-(float)(vehicle_info_pt-> veh_id - 1)*SPLIT_DIST;    
...
// declarations for timing checking                    // For DBG on Aug 6 03
	 ...
	 */ 
     ...
     if (config_pt-> task == 2)    
     {
	     ...
	     if (test_site == NVD)
	     {
	           if (config_pt-> run == 1)       // Low Variable Max Speed
	           {                           		
          			
          			if (vehicle_info_pt -> run_dist < 2100.0)         	
          	    		config.max_spd=(35.0)*mph2mps;           	    		
          	    	else if    (vehicle_info_pt -> run_dist < 2800.0)
          	    		config.max_spd=(25.0)*mph2mps;          	    		
          	   		else if    (vehicle_info_pt -> run_dist < 4500.0)
          	    		config.max_spd=(40.0)*mph2mps;	 
          	    	else if    (vehicle_info_pt -> run_dist < 5200.0)
          	    		config.max_spd=(30.0)*mph2mps;
          	   		else 
          	    		config.max_spd=(45.0)*mph2mps;
  	            }  // run 1 end
  	           
	           if (config_pt-> run == 2)         // High Variable Max Speed; time intervals have been adjusted OK.     03_27_11
	           {                           		
          			
          			if (vehicle_info_pt -> run_dist < 2200.0)         	
          	    		config.max_spd=(48.0)*mph2mps; 
          	    	else if    (vehicle_info_pt -> run_dist < 2600.0)
          	    		config.max_spd=(35.0)*mph2mps;
          	   		else if    (vehicle_info_pt -> run_dist < 4400.0)
          	    		config.max_spd=(50.0)*mph2mps;	 
          	    	else if    (vehicle_info_pt -> run_dist < 5000.0)
          	    		config.max_spd=(40.0)*mph2mps;
          	   		else 
          	    		config.max_spd=(53.0)*mph2mps;
  	           }  // run 2 end
...
if(pparams->denso_lidar == 1) {
     ...
     lid_hi_rg[0] = ((plidar_A->h_dist_1 << 8) | plidar_A->l_dist_1)*0.005;
     lid_hi_rg[1] = ((plidar_A->h_dist_2 << 8) | plidar_A->l_dist_2)*0.005;
     lid_hi_rg[2] = ((plidar_A->h_dist_3 << 8) | plidar_A->l_dist_3)*0.005;
     lid_hi_rg[3] = ((plidar_A->h_dist_4 << 8) | plidar_A->l_dist_4)*0.005;
     lid_hi_rg[4] = ((plidar_B->h_dist_5 << 8) | plidar_B->l_dist_5)*0.005;
     lid_hi_rg[5] = ((plidar_B->h_dist_6 << 8) | plidar_B->l_dist_6)*0.005;
     lid_hi_rg[6] = ((plidar_B->h_dist_7 << 8) | plidar_B->l_dist_7)*0.005;
     lid_hi_rg[7] = ((plidar_B->h_dist_8 << 8) | plidar_B->l_dist_8)*0.005;
...
if(pparams->denso_lidar == 1) {
   ...
   /*****************************************
                GPS and Road Grade
   *****************************************/
//  db_clt_read(pclt, DB_GPS_PT_LCL_VAR, sizeof(path_gps_point_t), &self_gps_point);   
   
   for (i=1; i<=(vehicle_info_pt-> pltn_size); i++)
   		road_info_pt-> postmile=(float)(EB_pstm_0 + (vehicle_info_pt  -> run_dist - ((float)(vehicle_info_pt-> veh_id-1))*(DES_FOLLOW_DIST+COMB_LENGTH))); 
    gps_trk_pt-> latitude = self_gps_point. latitude;
    gps_trk_pt-> longitude = self_gps_point. longitude;
    gps_trk_pt-> altitude = self_gps_point. altitude;
    gps_trk_pt-> speed = self_gps_point. speed;
    gps_trk_pt-> heading = self_gps_point. heading;
    
   if (config_pt-> task == 10)      
   {
     if ( rd_grade(rd_info_sw, vehicle_info_pt , gps_trk_pt, road_info_pt) != 1 )
        	fprintf(stderr, " Calling  rd_grade()  fail! \n");          
   }
...
if( (config.handle_faults == TRUE) && (maneuver_des > 1) && (maneuver_id[0] != 30))
{ 
   ...
   if (vehicle_info_pt-> pltn_size >= 1)       
    ...
    if (t_ctrl > 0.001)                                //05/05/10                  
    {
         ...
         {
            if (coording(dt, track_length, con_state_pt, config_pt, f_mode_comm_pt, vehicle_info_pt, manager_cmd_pt) != 1)
     ...
     if ( maneuver(dt, t_ctrl, time_filter, v_p, c, d, road_info_pt, config_pt, con_state_pt, vehicle_info_pt,  // time filter added on Dec. 1, 03
                 f_index_pt, maneuver_des, maneuver_id) != 1 )
    ...
    if ( (maneuver_des == 29) && ((con_state_pt-> pre_v < 3.0) || (v < 3.0))) 
         		maneuver_id[0] = 30;
     
    // For all vehicles, over write maneuver_des // 02_13_11; This may not be necessary
    /*
     if (pparams->vehicle_type == VEH_TYPE_TRUCK_BLUE) 
   		{
	   		if ( (maneuver_des == 29) && ((con_state_pt-> pre_v < 3.0) || (v < 3.0))) 
	...
	if (pparams->vehicle_type == VEH_TYPE_TRUCK_GOLD) 
   		{
	   		if ( (maneuver_des == 29) && ((con_state_pt-> pre_v < 6.0) || (v < 6.0))) // 1.2
         		maneuver_id[0] = 30;
   		} 

	if (pparams->vehicle_type == VEH_TYPE_TRUCK_SILVR) 
   		{
	   		if ( (maneuver_des == 29) && ((con_state_pt-> pre_v < 10.0) || (v < 10.0))) //1.2 
         		maneuver_id[0] = 30;
...
if( config.run_data == TRUE ) {
        ...
        fprintf(pout, "%4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f ",               
             ...
             pv-> fuel_rate                         //con_state_pt-> manu_acc            //48                       
        );         
...
if( config.read_data == TRUE ) {
        ...
        fprintf(pout, "%3i %3i %3i %3i %3i %3i %3i %3i %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %d %4.3f %4.3f\n",
             read_sw,                    //18
             eng_mode,                   //19
             jk_mode,                    //20
             ebs_brake_switch,           //21 
             ...
             pv-> mb_axle,               //40
             pv-> rb_axle,               //41
             pv-> fan_drive_state,                      // 42                                      // Added on 05/03/11
		     pv-> estimated_percent_fan_speed,   //43                             // Added on 05/03/11
             local_time                   //44
   		);   
}
