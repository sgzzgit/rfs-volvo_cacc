/*********************************************************************
       ...
       Fault management modified;                                          Aug. 21, 10
	 Some test tasks and runs have been defined for RFS and NVD;         Aug. 22, 10
...
//    unsigned short j=0;
 ...
 //   verbose_flag=TRUE;
    ...
    if( test_site == RFS && config_pt-> task == 1)
       {
          if (config.max_spd > 15.0) //26.0
             config.max_spd=15.0;
          config.max_spd=(config.max_spd)*0.4469;         
    ...
    else if( test_site == NVD && config_pt-> task == 1)
       {
          if (config.max_spd > 60.0)
             config.max_spd=60.0; 
          config.max_spd=(config.max_spd)*0.4469;
          //con_state. mag_space = 1.0;
          track_length = 6.0*1609.0;
          config.max_dcc=MAX_DCC;
       }
    else;
   ...
   // From veh_long.h
     ...
     if (test_site == RFS && config_pt-> task == 2)      
           {
              if (config_pt-> run == 1)
              {
                con_state. des_f_dist=10.0;                
          		...
          		con_state. mag_space = 1.0;
          		track_length = 250.0;   //350.0;        // [m]
          		if (vehicle_info_pt-> run_dist <= 80.0)
          			config.max_spd=(18.0)*0.4469;         
          		else if (vehicle_info_pt-> run_dist <= 150.0)
          			config.max_spd=(10.0)*0.4469;  
          		else 
          			config.max_spd=(15.0)*0.4469;            		
              }           
              if (config_pt-> run == 2)
              {
                con_state. des_f_dist=10.0;                
   ...
   if (test_site == NVD)
      {  
	     track_length = 6.0*1609.0; 
	     if (config_pt-> task == 1)    // task 1
	       {
		     if (config_pt-> run == 1)
		     {
		     	con_state. des_f_dist=10.0; 
		     	config.max_dcc=MAX_DCC;       		         		          		
             	config.max_spd=(35.0)*0.4469; 
             } 
             if (config_pt-> run == 2)
		     {
		     	con_state. des_f_dist=10.0; 
		     	config.max_dcc=MAX_DCC;       		         		          		
             	config.max_spd=(45.0)*0.4469; 
             } 
             if (config_pt-> run == 3)
		     {
		     	con_state. des_f_dist=10.0; 
		     	config.max_dcc=MAX_DCC;       		         		          		
             	config.max_spd=(55.0)*0.4469; 
             }   
             if (config_pt-> run == 4)
		     {
		     	con_state. des_f_dist=10.0; 
		     	config.max_dcc=MAX_DCC;       		         		          		
             	config.max_spd=(60.0)*0.4469; 
             }                                 
	       }    
	     if (config_pt-> task == 2)      // task 2
           {
              ...
              if (config_pt-> run == 1)
              {
                con_state. des_f_dist=10.0;                
          		config.max_dcc=MAX_DCC;       		         		
          		if (vehicle_info_pt-> run_dist <= 2000.0)
          			config.max_spd=(35.0)*0.4469;         
              ...
              if (config_pt-> run == 2)
              {
                con_state. des_f_dist=10.0;                
          		...
          		if (vehicle_info_pt-> run_dist <= 2000.0)
          			...
          			config.max_spd=(25.0)*0.4469;  
          		else if (vehicle_info_pt-> run_dist <= 4000.0)
          			config.max_spd=(55.0)*0.4469;  
          		else if (vehicle_info_pt-> run_dist <= 5000.0)
          		    config.max_spd=(45.0)*0.4469; 
          		else if (vehicle_info_pt-> run_dist <= 6000.0)
          		    config.max_spd=(60.0)*0.4469;
          		else   
          		    config.max_spd=(55.0)*0.4469;   		
              }                                                                       
           }
         if (config_pt-> task == 3)            // task 3
           {
          		...
          		if  (config_pt-> run == 1) 
          			config.max_spd=(35.0)*0.4469;     		         		          			
          		else if  (config_pt-> run == 2)     		         		
          			config.max_spd=(45.0)*0.4469;  
          		else if  (config_pt-> run == 3)     		         		
          			config.max_spd=(50.0)*0.4469;  
          		else   		         		
          			config.max_spd=(55.0)*0.4469;           		 	 
           }
         if (config_pt-> task == 4)            // task 4
           {	                       
          		config.max_dcc=MAX_DCC;
...
if( (config.handle_faults == TRUE) && (maneuver_des > 0) )   // >1 before; 08_23_10
{ 
   ...
   if (vehicle_info_pt-> pltn_size >= 1)       
     {
	    if (vehicle_info_pt-> veh_id == 1)
	    ...
	    else
	       {
               ...
               if ( (f_index_pt-> comm == 1) || ((f_index_pt-> e_vrd==1)&&(f_index_pt-> lidar==1)) || (sw_pt->brake_sw == 1) )                                            
                  vehicle_info_pt-> fault_mode = 3;                     		          
           }       
     } 
     ...
     if ( (f_index_pt-> comm == 1) || (vehicle_info_pt-> fault_mode == 3) )
...
if( config.run_data == TRUE ) {
        ...
        fprintf(pout,"%4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f ",  
             lid_hi_rg[0], //con_output_pt-> y7,                //27   
             lid_hi_rg[1], //con_output_pt-> y8,                //28  
             lid_hi_rg[2], //con_output_pt-> y9,                //29  
             lid_hi_rg[3], //con_output_pt-> y10,               //30
             lid_hi_rg[4], //con_output_pt-> y11,               //31  
             lid_hi_rg[5], //con_output_pt-> y12,               //32  
             lid_hi_rg[6], //con_output_pt-> y13,               //33  
             lid_hi_rg[7], //con_output_pt-> y14,               //34    
