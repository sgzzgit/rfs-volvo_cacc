...
******************************************************************************/
    ...
    {  
	    ...
	    if (handshake_start_arr[1] == ON)  
           ...
           {	                             
               ...
               if (comm_f_counter[1] > comm_err_bound)
               {
        ...
        if (handshake_start_arr[2] == ON)
        ...
        else
           {
           	   ...
           	   if (comm_f_counter[2] > comm_err_bound)
           	   {
        ...
        if (handshake_start_arr[3] == ON)
           ...
           {  
               ...
               if (comm_f_counter[3] > comm_err_bound)
               {
	...
	for(pip = 1; pip <= vehicle_info_pt->pltn_size; pip++)      // Problem here: 
	{
		if(pip != vehicle_pip) 		 
		{
			handshake_start &= handshake_start_arr[pip];        // assign only when all the others are true
			comm_info_pt->comm_reply &= comm_reply_arr[pip];    // assign only when all the others are true
