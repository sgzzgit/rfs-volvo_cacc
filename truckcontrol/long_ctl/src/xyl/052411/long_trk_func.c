...
//                     con_state_pt-> pre_mag_counter = comm_receive_pt1-> marker_counter;    // Should come from communication.
		...
		{
			vehicle_info_pt-> fault_mode=3;                  // added on 09_13_10
			vehicle_info_pt-> pltn_fault_mode=3;
		}
...
******************************************************************************/
    ...
    v2 = pv->vehicle_speed;                      // From Sue's Code, Feb. 25 03
     if (pparams->vehicle_type == VEH_TYPE_TRUCK_BLUE) {          
        v1 = pv-> front_axle_speed*0.979783236; //0.9735; //1.03306; //0.979783236;    // 0.979717*2342.5/2340.0; For rented trailer, mass_sw == 6; 12_02_03 //According to the survey by Chris Cherry
        }
     if (pparams->vehicle_type == VEH_TYPE_TRUCK_GOLD) {
        v1 = pv-> front_axle_speed*0.975548*(2347.8/2340.0)*(1.0-10.0/(5.0*1609.0)); //For rented trailer, 12_02_03  //*0.9981 for gl_rent; //*0.997;                 //0.9921;
               // from ebc2 on D1
        }                                                         
     if (pparams->vehicle_type == VEH_TYPE_TRUCK_SILVR) {
        v1 = pv-> front_axle_speed*0.9776258546*(1.0+35.0/(5.0*1609.0)); // 0.9776  //0.9776258546;
        }   
...
******************************************************************************/
     if (con_output_pt-> con_sw_1 == 1)  // Tq cmd
      {
                   ...
                   {                            
                      if (maneuver_id[0] == 7)
                        ...
                        {                               
                                ...
                                if (tmp_rate < -0.65)
                                tmp_rate = -0.65;
                                pcmd->engine_torque = (con_output_pt-> y1)*(1.0+1.15*tmp_rate);    // Modified on 09_10_10
                                //pcmd->engine_torque = (con_output_pt-> y1)*(1.0+tmp_rate_v);    // Modified on 09_15_10
                        }
                   }
                else
                   {
                        ...
                        pcmd->engine_torque = (con_output_pt-> y1)*(1.0+tmp_rate);    // Modified on 09_10_10
                        //pcmd->engine_torque = con_output_pt-> y1;    // default
                       // if ( pcmd->engine_torque > 0.0)    // removed on 05_24_11
                       //     pcmd->engine_torque= pcmd->engine_torque*0.3;
                   }
                }    // SILVR end
          } // NVD end
          if (test_site ==RFS)
          ...
          if (pcmd->engine_torque < minimum_torque)
      ...
      {   
	      ...
	      if (test_site == NVD)
	      {
			 ...
			 if (pparams->vehicle_type == VEH_TYPE_TRUCK_SILVR)  
			 {
                ...
                //pcmd->engine_retarder_torque = -con_output_pt-> y17; // Negative torque, not percentage; changed 09_13_10
                pcmd->engine_retarder_torque = (-con_output_pt-> y17)*(1.0-1.25*tmp_rate); // Negative torque, not percentage
                */                                                                                                                                                                                                   //   Removed on 05_22_11
                  if (tmp_rate > 0.45)
                    tmp_rate = 0.45;    
                  if (tmp_rate < -0.65)
      ...
      if (con_output_pt-> con_sw_4 == 1)  //trans rtd cmd          
      {
	       ...
	       if (test_site == NVD)  
	       {
			  ...
			  if (pparams->vehicle_type == VEH_TYPE_TRUCK_GOLD)
				{
                    ...
                    if (tmp_rate < -0.55)
                      tmp_rate = -0.55;	  
                    //pcmd->trans_retarder_value = -(con_output_pt-> y10); // Negative percentage;  changed 09_13_10                                                        
			        pcmd->trans_retarder_value = -(con_output_pt-> y10)*(1.0-tmp_rate);
				}  // Gold end
		    ...
		    if (test_site == RFS)	
		    	pcmd->trans_retarder_value = -(con_output_pt-> y10); 
              pcmd->engine_command_mode = TSC_OVERRIDE_DISABLED;                 
      }                                                        
      else                                                         
      {                                                        
                 ...
                 if(con_state_pt-> pre_v < 0.8*(config_pt-> max_spd))                                           //0.8 used on 10_30,03                                                        
                  ...
                  if (con_state_pt-> pre_v < 4.0)     //2.0        
                    {                                              
                       ...
                       pcmd->brake_command_mode = EXAC_ACTIVE;
                       if (pparams->vehicle_type == VEH_TYPE_TRUCK_SILVR)    
                       	pcmd->ebs_deceleration = - 0.85*STOP_BRAKE;
                       else
   ...
   //Use this if possible                                          
           ...
           else    // For 2nd and 3rd veh                                                
              {   
	             if (vehicle_pip == 2)	             
					{
						if(con_state_pt-> pre_v < 0.6*(config_pt-> max_spd)) // Added on 02_12_11                                                        
                           {
                             pcmd->engine_command_mode = TSC_OVERRIDE_DISABLED;
                             pcmd->engine_torque = 0.0;                
                           } 
				 ...
				 if (vehicle_pip == 3)
				    {                                                
                       if(con_state_pt-> pre_v < 0.8*(config_pt-> max_spd))  // Added on 02_12_11                                                        
                          {
                             pcmd->engine_command_mode = TSC_OVERRIDE_DISABLED;
                             pcmd->engine_torque = 0.0;                
                          }    
                 ...
                 if (con_state_pt-> pre_v < 4.0)     //2.0         
                    ...
                    {                                              
                       ...
                       pcmd->brake_command_mode = EXAC_ACTIVE;   
                       if (pparams->vehicle_type == VEH_TYPE_TRUCK_SILVR)    
                       	pcmd->ebs_deceleration = - 0.65*STOP_BRAKE;
                       else                       
      ...
      if (maneuver_id[0] == 30)                                    
        {                                                          
          ...
          pcmd->brake_command_mode = EXAC_ACTIVE; 
          if (pparams->vehicle_type == VEH_TYPE_TRUCK_SILVR)    
            pcmd->ebs_deceleration = - 1.55*STOP_BRAKE;
          else                       	              
...
||    
        ...
        if(config->eaton_radar == TRUE) {
          ...
          //      fprintf(stderr, "Calling radar_filter fail! \n");
            ...
            if( (global_time < 10.0) && (con_state_pt-> vrd_range > 15.0) ) 
			   con_state_pt->vrd_range  = 6.0;     // 10.0
...
//               fprintf(stderr, " Calling Lidar_filter fail! \n");
            ...
            con_state_pt-> lidar_range=0.85*(ldr_out_pt-> long_pos);     
            
             if( (global_time < 10.0) && (con_state_pt-> lidar_range > 15.0) ) 
			con_state_pt-> lidar_range  = 6.0;     // 10.0
...
//            con_state_pt-> lidar_range=(ldr_out_pt-> long_pos);     
	    ...
	    if( (global_time < 10.0) && (ldr_mdl.rg > 15.0) ) 
            ...
            con_state_pt-> lidar_range=0.85*(con_state_pt-> mdl_rg);
