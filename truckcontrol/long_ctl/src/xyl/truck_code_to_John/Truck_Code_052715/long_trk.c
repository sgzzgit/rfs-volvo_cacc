...
//	    &minimum_torque, &inactive_ctrl, manager_cmd_pt, sw_read_pt, vehicle_info_pt, jbus_read_pt);
 ...
 if ( (jbus_read_pt-> v > 11.0) || (sw_rd_pt->  CC_enable_sw == 0) || (sw_pt-> brk_sw ==1))
 {
	 pcmd->engine_torque=0.0;
	 pcmd->engine_torque = TSC_OVERRIDE_DISABLED;
 }
...
if( config.run_data == TRUE ) {
        fprintf(pout, "%02d:%02d:%02.3f %3i %3i %3i %3i %3i %3i %3i %4.3f %4.3f %4.3f %4.3f %4.3f %4.3f ",
             timestamp->tm_hour,
             timestamp->tm_min,
             seconds,                         
             maneuver_id[0],                                  
