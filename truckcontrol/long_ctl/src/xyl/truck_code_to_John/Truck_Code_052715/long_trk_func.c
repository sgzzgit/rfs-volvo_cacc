...
*******************************************************************************
******************************************************************************/
int actuate(long_output_typ *pcmd, float *pengine_reference_torque, float *pt_ctrl, int *maneuver_id,
     ...
     if (con_output_pt-> con_sw_1 == 1)  // Tq cmd
      ...
      {   
	      ...
	      if (test_site == NVD)
	      {
			 ...
			 if (pparams->vehicle_type == VEH_TYPE_TRUCK_VOLVO_2)
			 {
				 tmp_rate=(con_state_pt->front_range - con_state_pt-> temp_dist)/(con_state_pt-> des_f_dist);  
	             if (tmp_rate > 0.65)
                     tmp_rate = 0.65;	
			 ...
			 if (pparams->vehicle_type == VEH_TYPE_TRUCK_VOLVO_3)  
			 {
				tmp_rate=(con_state_pt->front_range - con_state_pt-> temp_dist)/(con_state_pt-> des_f_dist);            
                                                                                                                                                                                           //   Removed on 05_22_11
                  if (tmp_rate > 0.45)
...
int set_init_leds(db_clt_typ *pclt, unsigned short *pprt1_sw, 
	...
	return 0;     //TAKE THIS OUT LATER IF AND WHEN WE GET LEDS!!!
