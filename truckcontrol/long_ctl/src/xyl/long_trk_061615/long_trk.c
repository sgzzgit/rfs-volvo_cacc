...
// declarations for timing checking                    // For DBG on Aug 6 03
	 ...
	 */ 
     ...
     if( test_site == RFS )
     {
		...
		// manager_cmd_pt-> man_des=4;

		 if (config_pt-> task == 1)    
		 {
			
			track_length = 350.0;                           	    
			config.max_spd=28.0*mph2mps;  
	              		
			if (config.max_spd > 30.0*mph2mps)      
...
if(config.use_comm == TRUE) 
{
   ...
   ************************************************/
	...
	con_state_pt-> vrd_range=0.0;
	con_state_pt-> vrd_range_rate=0.0;
...
actuate(pcmd, con_output_pt, con_state_pt, pparams, &inactive_ctrl, manager_cmd_pt, sw_read_pt, jbus_read_pt);
/*
mytimer += 0.02;

if (con_output_pt-> y1 > 0.0)
{
 pcmd->engine_command_mode = TSC_TORQUE_CONTROL;

pcmd->engine_torque=(con_output_pt-> y1)/ENGINE_REF_TORQUE;
...
//if( ((++my_torque_counter) % 100) == 0)
// 	pcmd->engine_torque= OPEN_LOOP_TQ - 1.0;
//else
// 	pcmd->engine_torque= OPEN_LOOP_TQ;

 if ( (jbus_read_pt-> v > 20.0) || (sw_read_pt->  CC_enable_sw == 0) || (sw_read_pt-> brk_sw ==1))
 {
	 pcmd->engine_torque=0.0;
	 pcmd->engine_command_mode = TSC_OVERRIDE_DISABLED;
 }

if(mytimer >= 35 || (vehicle_info_pt-> run_dist > stop_dist)) {
	pcmd->engine_retarder_command_mode = TSC_TORQUE_CONTROL;   // Use this if possible
	pcmd->engine_retarder_torque = -95.0;
	pcmd->engine_torque=0.0;
//	pcmd->engine_command_mode = TSC_OVERRIDE_DISABLED;
	if(do_once == 0)
	//printf("Retarder enabled at %f%%\n", pcmd->engine_retarder_torque);
	do_once = 1;
	pcmd->ebs_deceleration=0.85;
}
