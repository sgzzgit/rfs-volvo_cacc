...
from-env "AVCS_POSITION" = '3' ;
from-env "AVCS_VEHICLE" = 'Silvr' ;
...
#	slay  -f -Q -s TERM rdj1587
	...
	sleep 1
	$TEST_DIR/trk_cr -t 1000 2>long_input.log &
	sleep 1
	...
	$TEST_DIR/jbussend -c -e /dev/can1 -b /dev/can2 >/home/truckcontrol/test/jbussend.log &
	sleep 1
	$TEST_DIR/gpsrcv -n 202 -u 5050  >gpsrcv.log &
	$TEST_DIR/veh_rcv -f $CONFIG_FILE -u 5051  -t Silvr >veh_rcv.log &
	$TEST_DIR/veh_snd -i 20 -a $DSRC_IP -u 5052  -t Silvr >veh_snd.log &
	sleep 1
	$CAN_CLIENT_DIR/rdj1939 -c -f /dev/can1 >j1939_eng.log &
	sleep 2
	$CAN_CLIENT_DIR/rdj1939 -c -f /dev/can2 >j1939_brake.log &
	sleep 2
	$TEST_DIR/evt300 -s "/dev/ser1" -r a -o 2 >evt300a.log &
	sleep 1
#	$TEST_DIR/rdlidar -o 2 < /dev/ser2 >densolidar.log &
#	sleep 1
	$TEST_DIR/mdl -o 2 < /dev/ser2 >mdl.log &
	sleep 1
	...
	sleep 1
#	DATEDIR=`date +%m-%d-%Y`
#	if [[ ! -d /big/data/$DATEDIR ]]
#	then
#		mkdir /big/data/$DATEDIR
#	fi
%)
;
...
%(
	...
	/home/truckcontrol/test/$LONGITUDINAL -v Silvr -f $LONG_DIR$LONG_CFG_FILE.ini -o /big/data/$LONG_CFG_FILE -r $COMMAND_TEST >$LONG_CFG_FILE$COMMAND_TEST.log &
%)
;
...
%(
	$TEST_DIR/trk_wr -t 50 1>$DATAFILE 2>long_input.log &
%)
;
shell 'Stop trace file'
%(
	slay -f -Q -s TERM trk_wr
	echo Notes for trk_wr \(end with <CR><ctrl-d>\)?
	cat >>/big/data/trk_wr.dat
	echo Suffix for trk_wr \(end with <CR><ctrl-d>\)?
	cat >/big/data/trk_wr.dat.suffix
	mv /big/data/trk_wr.dat /big/data/trk_wr.dat.`cat /big/data/trk_wr.dat.suffix`
	mv /big/data/test.dat /big/data/test.dat.`cat /big/data/trk_wr.dat.suffix`
%)
;
skip ;
