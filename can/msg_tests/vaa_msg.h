...
typedef struct {
	unsigned long vaa_msg_id;		// add to DB_VAA_BASE to get DB VAR number
				// send as ID in CAN message
	vaa_rcv_fptr rcv;
	vaa_snd_fptr snd;
	vaa_print_fptr print;
	int vaa_msg_size;	// in bytes of underlying structure
} IS_PACKED vaa_msg_funcs_t;

extern vaa_msg_funcs_t vaa_msg_funcs[];
