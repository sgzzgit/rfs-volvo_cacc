...
SIGEV_PULSE_INIT(&pulse_event, coid,  20, _PULSE_CODE_MINAVAIL, 0);
...
DeRegRdPulse(hdl);  // de-register current notification pulse
RegRdPulse(hdl, &pulse_event);

for  (;;)
{
// a pulse will be  triggerd after a frame or more has been received
rcvid = 0; //MsgReceivePulse(chid, &pulse, sizeof(struct  _pulse), NULL);
if(rcvid == -1)
perror("recv\n");
else
{
...
if(resp == 0 || resp  == -12)
{
printf("CAN Status: %04x %d",  CanGetStatus(hdl, &st),
