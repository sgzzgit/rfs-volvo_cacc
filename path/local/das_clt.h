 ...
 *	das_clt.h
 *
 *	Definitions common to client and DAS I/O management server.
...
/** Used by scan and read operations where buffer for send or
 * receive is passed to the function that does the message send to 
 * the DAS manager.
 */
typedef struct 
{
	io_msg_t io;
	short int n;	/// used for number of channels or entries
} das_msg_t;
