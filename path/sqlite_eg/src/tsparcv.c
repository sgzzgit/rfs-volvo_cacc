/**\file 
 *
 * tsparcv.c    Reads signal countdown message with TSP information
 *		and writes to database for TSP announcment.
 *		
 *	
...
{	
	...
	/// Set dosound when tsptype 2 or 3, or when green
	/// and countdown less than 5 and not didsound
        if (pp->signal_state == 1) {	/// GREEN
		if ((pp->reqst_type == 2) || (pp->reqst_type == 3)
			|| (pp->time_to_next <= 5)) 
			pp->dosound = 1;
	} else {	/// turn off sound if not GREEN	
			pp->dosound = 0;
	}
	return (TRUE);
}
...
void tsp_msg_sql_update_str(char *cmd_str, tsp_msg_typ *pm, int link_id)
{
	...
	"phase=%d,countdown=%d,tsptype=%d,bustimesaved=%d,canPassOrNot=%d,dosound=%d",
			pm->signal_state,
			pm->time_to_next,
			pm->reqst_type,
			pm->bus_time_saved,		
...
int main(int argc, char *argv[])
{
	...
	unsigned int socklen;
        sqlite3 *sqlt;
        char *sqlt_name = "tspa.db";/// default name for sqlite database
