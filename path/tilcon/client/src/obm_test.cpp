/**\file
 *
 *	obm_test.cpp	Test Tilcon vertion 5.5 for On-Board Monitoring project
 *
...
void obm_test_set_tilcon(pid_t cid, void *data);
...
int main(int argc, char *argv[])
{
	...
	char *obm_tilcon_dir = "/home/path/tilcon/client/twd";
	...
	veh_stat_callback.set_values = obm_test_set_tilcon;
	veh_stat_callback.data = NULL;

	if (errorcode = tilcon_init_app(tilcon_cid, 2, obm_tilcon_dir, 
...
{
	tilcon_init_meter(cid, &meter_w, "meter", 50, 1, AMBER);
	tilcon_init_meter(cid, &needle_w, "needle", 50, 1, AMBER);
	tilcon_init_multistate(cid, &radar_w, "radar", 0);
}

/**
 * void obm_test_set_tilcon()
 *
...
void obm_test_set_tilcon(pid_t cid, void *data)
{
	...
	db_clt_read(pclt, DB_OBM_RADAR_VAR, sizeof(db_radar), &db_radar);
	db_clt_read(pclt, DB_OBM_METER_VAR, sizeof(db_meter), &db_meter);
	tilcon_set_multistate(cid, &radar_w, db_radar.radar_status);
	tilcon_set_meter(cid, &meter_w, db_meter.meter_val,
				db_meter.meter_fill); 
	tilcon_set_meter(cid, &needle_w, db_meter.meter_val,
				TRT_Transparent); 
}
