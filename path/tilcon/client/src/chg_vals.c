...
static db_id_t db_vars_list[] = {
	{DB_RADAR_VAR, sizeof(radar_typ)},
	{DB_DRAWING_VAR, sizeof(drawing_typ)},
	{DB_METER_VAR, sizeof(meter_typ)}};
...
int main(int argc, char *argv[])
{
	...
	db_clt_write(pclt, DB_OBM_RADAR_VAR, sizeof(radar_typ), &db_radar);

	db_meter.meter_val = 75.0;
	db_meter.meter_fill = color_array[0];
	db_meter.alarm_mode = 0;
	color_choice = 0;
	db_clt_write(pclt, DB_OBM_METER_VAR, sizeof(meter_typ), &db_meter);
	...
	for(;;) {
		...
		printf("6) Set meter alarm status to 1\n");
		printf("7) Set meter alarm status to 0\n");
		printf("8) Change drawing line color \n");
		printf("9) Change drawing fill color \n");
		scanf("%d", &value);
		switch (value) {
		case 1:
			db_radar.radar_status = 1;
			db_clt_write(pclt, DB_RADAR_VAR,
				 sizeof(radar_typ), &db_radar);
			break;
		case 2:
		...
		case 3:
			...
			db_clt_write(pclt, DB_METER_VAR,
				 sizeof(meter_typ), &db_meter);
			break;
		case 4:
			db_meter.meter_val -= 5.0;
		...
		case 8:
			line_choice += 1;
			if (line_choice == max_colors)
				line_choice = 0;
			db_drawing.line_color = color_array[line_choice];
			db_clt_write(pclt, DB_DRAWING_VAR,
				 sizeof(drawing_typ), &db_drawing);
			break;
		case 9:
			fill_choice += 1;
			if (fill_choice == max_colors)
				fill_choice = 0;
			db_drawing.fill_color = color_array[fill_choice];
			db_clt_write(pclt, DB_DRAWING_VAR,
				 sizeof(drawing_typ), &db_drawing);
			break;
		default:
